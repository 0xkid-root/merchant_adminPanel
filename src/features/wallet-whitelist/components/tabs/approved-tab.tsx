...
export function ApprovedTab() {
  ...
  const { data, isLoading } = useWhitelistRequests(
    { page, size: 10 },
    { status: "Approved", search: debouncedSearch }
  );
        ...
        <Table>
          <TableHeader className="bg-transparent">
            <TableRow className="hover:bg-transparent border-slate-100 dark:border-slate-800">
              ...
              <TableHead className="h-11 px-5 text-[11px] font-semibold uppercase tracking-wider text-slate-500">Account Holder</TableHead>
              <TableHead className="h-11 px-5 text-[11px] font-semibold uppercase tracking-wider text-slate-500">Bank</TableHead>
              <TableHead className="h-11 px-5 text-[11px] font-semibold uppercase tracking-wider text-slate-500">Account Number</TableHead>
              <TableHead className="h-11 px-5 text-[11px] font-semibold uppercase tracking-wider text-slate-500">IFSC</TableHead>
              <TableHead className="h-11 px-5 text-[11px] font-semibold uppercase tracking-wider text-slate-500 text-center">Document</TableHead>
              <TableHead className="h-11 px-5 text-[11px] font-semibold uppercase tracking-wider text-slate-500">Approved On</TableHead>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={9} className="h-32 text-center text-slate-500">
                  {debouncedSearch ? "No requests match your search" : "No approved wallet whitelist requests found"}
                </TableCell>
              </TableRow>
            ) : data?.data.map((req) => (
              <TableRow key={req.id} className="border-slate-100/60 dark:border-slate-800/60 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                ...
                <TableCell className="p-4 px-5 font-medium text-slate-900 dark:text-white text-[13px]">
                ...
                <TableCell className="p-4 px-5">
                  <span className="font-mono text-[13px] font-medium text-slate-600 dark:text-slate-400">
