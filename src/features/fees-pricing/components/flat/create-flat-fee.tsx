...
const formSchema = z.object({
  ...
  flatFee: z.coerce.number().min(0, "Flat fee cannot be negative"),
...
export function CreateFlatFee() {
  ...
  const watchFlatFee = watch("flatFee");
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      console.log("Create Flat Fee:", data);
      toast.success("Flat fee created successfully");
      router.push("/fees-pricing/flat");
    } catch (error) {
      toast.error("Failed to create flat fee");
    } finally {
