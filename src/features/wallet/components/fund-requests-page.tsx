...
import { useFundRequestsQuery, useApproveFundRequest, useRejectFundRequest } from "../queries";
import { Search, Filter, Download, ChevronLeft, ChevronRight, CheckCircle2, XCircle, Eye } from "lucide-react";
...
export function FundRequestsPage() {
  const { data: requests, isLoading } = useFundRequestsQuery();
  ...
  return (
    <div className="flex flex-col gap-6 pb-8">
      <PageHeader
        title="Fund Requests"
        description="Review and process manual wallet deposit requests."
        actions={
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 rounded-xl border border-emerald-200/60 bg-emerald-50/50 shadow-sm dark:bg-emerald-900/10 dark:border-emerald-900/30">
          <p className="text-sm font-medium text-emerald-600 dark:text-emerald-500">Approved</p>
          <p className="text-2xl font-bold mt-1 text-emerald-700 dark:text-emerald-400">{approved}</p>
        </div>
        <div className="p-4 rounded-xl border border-red-200/60 bg-red-50/50 shadow-sm dark:bg-red-900/10 dark:border-red-900/30">
          <p className="text-sm font-medium text-red-600 dark:text-red-500">Rejected</p>
          <p className="text-2xl font-bold mt-1 text-red-700 dark:text-red-400">{rejected}</p>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col sm:flex-row items-center gap-4 justify-between">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="relative w-full sm:w-80">
              ...
              <Input 
                placeholder="Search request ID, merchant, ref..." 
          ...
          <Table>
            <TableHeader className="bg-transparent">
              <TableRow className="hover:bg-transparent border-slate-100 dark:border-slate-800">
                ...
                <TableHead className="h-11 px-5 text-xs font-semibold uppercase tracking-wider text-slate-500">Payment Method</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow key={req.id} className="border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                  ...
                  <TableCell className="p-4 px-5">
                    <span className="text-[13px] text-slate-600 dark:text-slate-400">{req.paymentMethod}</span>
                  </TableCell>
                  <TableCell className="p-4 px-5">
                    <span className="font-mono text-[12px] text-slate-500">{req.reference || "—"}</span>
                  </TableCell>
      ...
      <Dialog open={!!dialogAction} onOpenChange={(open) => !open && setDialogAction(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {dialogAction === "approve" ? "Approve Fund Request?" : "Reject Fund Request?"}
            </DialogTitle>
            <DialogDescription>
              {dialogAction === "approve" 
                ? "This will credit the funds to the merchant's wallet immediately."
                : "This will decline the request. The merchant will need to submit a new one."}
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedRequest && (
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg p-4 my-4 space-y-3">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-mono text-slate-900 dark:text-white">{selectedRequest.reference}</span>
              </div>
            </div>
          )}
